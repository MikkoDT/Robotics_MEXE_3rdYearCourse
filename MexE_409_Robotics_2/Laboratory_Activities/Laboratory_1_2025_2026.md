...
| Intended Learning Outcome (ILO) | Exemplary (100%-95%) | Proficient (94%-85%) | Competent (84%-75%) | Developing (74%-65%) | Needs Improvement (64%-50%) | Poor/Missing (< 50%) | Max Points |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :---: |
| **ILO1: Kinematics (5 pts)** | Flawless diagram, D-H table, and matrices. | Minor labeling error, perfect matrices and D-H parameters. | Minor arithmetic errors in matrices, correct D-H parameters. | Some incorrect D-H parameters affecting the final matrices. | Major errors in D-H parameter extraction or matrix derivation. | Missing or fundamentally flawed. | **5** |
| **ILO2: Analysis (10 pts)** | Accurate Jacobian & singularity analysis for *both* 3-DOF and 6-DOF. | Accurate Jacobian, but misses minor singularities in the 6-DOF analysis. | Minor mathematical errors in either the 3-DOF or 6-DOF analysis. | Completes 3-DOF perfectly, but struggles significantly with 6-DOF logic. | Completes analysis for 3-DOF only; missing 6-DOF entirely. | Fails to compute Jacobian or analyze motion. | **10** |
| **ILO3: Simulation (25 pts)** | Flawless Python/MATLAB execution for *both* 3-DOF and 6-DOF. Output matches perfectly. | Codes run flawlessly for both, minor inefficiencies or variable naming issues. | Codes run for both but have minor bugs or slight output discrepancies between software. | Successful 3-DOF simulation, but 6-DOF has major logical errors or fails to execute properly. | Fails to build the 6-DOF simulation completely or significant bugs in both languages. | Programs fail to run; missing either Python or MATLAB. | **25** |
| **ILO4: Presentation (20 pts)** | Perfect GitHub repository. Strict 10-min presentation with strong mathematical verification and Q&A. | Great repository. Presentation is clear but slightly off timing, good Q&A. | Good repo but missing minor formatting. Presentation lacks strong mathematical verification. | Disorganized repository. Presentation flow is poor, team struggles in Q&A. | Unorganized repo. Exceeds time limits significantly, poor Q&A performance. | Repo not submitted or presentation unprepared. | **20** |
